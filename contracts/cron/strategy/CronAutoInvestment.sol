...
contract CronAutoInvestment is AdminableInit, LPableInit {
    ...
    function addLiquidity(address[] calldata tokens, uint256[] calldata amountsDesired) public returns (uint256) {
    ...
    function removeLiquity(address to, uint256 liquidity) public returns (uint256 amountA, uint256 amountB) {
        // remove liquidity
    ...
    function deposite(uint256 amount) internal {
        if (amount > 0) {
    ...
    function withdraw(address to, uint256 liquidity) internal returns (uint256) {
        uint256 lpBalance = IERC20(pair).balanceOf(address(this));
    ...
    function doHardWork() public {
        ...
        uint256[] memory amountsDesired = new uint256[](1);
