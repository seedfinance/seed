...
library SwapLibrary {
        ...
        )
    {
        ...
        if (token != token0) {
            (address factory0, address[] memory path0) =
                manager.pathFor(token, token0);
            uint256[] memory amounts =
                UniswapV2Library.getAmountsOut(factory0, swapAmount, path0);
        ...
        if (token != token1) {
            (address factory1, address[] memory path1) =
                manager.pathFor(token, token1);
            uint256[] memory amounts =
                UniswapV2Library.getAmountsOut(factory1, swapAmount, path1);
